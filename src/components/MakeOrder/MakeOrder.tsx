...
const MakeOrder: React.FC<amountProp> = ({ amount, orderPage, onClick }) => {
  const deliveryMethod = useSelector(
    (state: RootState) =>
      state.order.deliveryMethod ?? "No delivery method selected"
  );
  const paymentMethod = useSelector(
    (state: RootState) =>
      state.order.paymentData?.method ?? "No payment method selected"
  );
      ...
      <div className={MakeOrderCss.form}>
        <div className={MakeOrderCss.form__info}>
          <div className={MakeOrderCss.form__line}>
            ...
            <span
              className={`${MakeOrderCss.lineRight} ${MakeOrderCss.price_style}`}
            >
              {amount}
            </span>
          </div>
        ...
        <form action="#" className={MakeOrderCss.form__submit}>
          ...
          <div className={MakeOrderCss.checkbox}>
            ...
            <label className={MakeOrderCss.labelText} htmlFor="button">
              By clicking on the
              <a className={MakeOrderCss.PlaceOrderLink} href="#">
                “Place an order”
              </a>{" "}
              button, you consent to the processing of personal data
            </label>
          </div>
        </form>
      </div>
